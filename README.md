...
`access_log` — окремий журнал: він отримує один новий запис для кожного фактичного входу, не впливає на схвалення і не використовується для пошуку доступу. У вкладці **«Журнал входів»** для нових сесій показується час, проведений у відкритому каталозі; хрестик видаляє запис одразу з інтерфейсу та БД.

## ⚙️ Таблиця `app_settings` в Supabase

Кольори брендів і налаштування каталогу зберігаються в `app_settings`, щоб вони однаково працювали на всіх пристроях.
...
supabase/migrations/20260916000000_access_log_session_duration.sql
```
...
## Переоцінка та «Coming soon»

В адмінці відкрийте **Переоцінка**. Таблиця завантажує всі товари, включно з прихованими, і показує їх по 60 на сторінці. Є пошук за назвою/штрихкодом, категорії, фільтр змінених рядків і різниця цін. Чернетки зберігаються при перемиканні вкладок адмінки; застосування — кнопкою **Зберегти ціни**. Не закривайте адмінку до збереження.

Для Excel:

1. Завантажте `.xlsx` або `.xls` (до 10 МБ, 20 000 рядків на аркуші). Шаблон доступний у вкладці.
2. Оберіть аркуш і колонки штрихкоду та вартості, вкажіть наявність заголовків. Стандартні заголовки визначаються автоматично.
3. Натисніть **Перевірити збіги**: валідні зміни з’являться у таблиці, а звіт покаже пропущені рядки.
4. Перегляньте й натисніть **Зберегти ціни**. Усі картки з відповідним `sku` отримають нову ціну; нові товари не створюються. Якщо частина записів не збережеться, залишаться лише їхні чернетки для повтору.

Штрихкоди зі стартовими нулями зберігайте в Excel як текст. Повторювані штрихкоди у файлі пропускаються, щоб уникнути неоднозначності. Ціна — невід’ємне число до 2 знаків після коми (кома або крапка); порожні й некоректні ціни пропускаються. Сам файл обробляється локально у браузері. Використовується [SheetJS CE з офіційного джерела](https://docs.sheetjs.com/docs/getting-started/installation/frameworks/), завантаження модуля відбувається лише при роботі з Excel.

У **Деталі картки → badge** є кнопка **✦ Coming soon**. Також цей badge можна обрати під час створення товару. Товар залишається видимим, ціна і кнопки кошика приховані; у картці показується повідомлення «Незабаром у продажу». Збережена ціна залишається доступною адміністратору для майбутнього продажу. Імпорт цін не змінює badge. Кошик синхронізує актуальні ціни та прибирає товари `Coming soon` після оновлення каталогу; перед оформленням повторно перевіряється доступність і ціна в базі.

Перевірки: `npm test` та `npm run build`. Тести охоплюють реальні XLS/XLSX, збіги й дублікати штрихкодів, некоректні ціни, часткові помилки запису, відображення бейджа та відхилення застарілого кошика.
